...
{ 
  ...
  public function getAllMenus($data){ 
    try{
      extract($data);
      $sql = "SELECT * FROM ".DB_PREFIX."header_menus where menu_status  != '9' ORDER BY menu_position";      
  ...
  public function getAllPages($data){ 
   ...
   public function updateMenuById($data){ 
    try{ 
      extract($data);
      $sql = "UPDATE ".DB_PREFIX."header_menus SET menu_parentid = :menu_parentid, menu_position = :menu_position WHERE menu_id = :menu_id"; 
  ...
  public function addMenuDetails($data){ 
    try{ 
      ...
      $sql = "INSERT INTO ".DB_PREFIX."header_menus(menu_name,menu_type,menu_module,menu_status) VALUES (:menu_name,:menu_type,:menu_module,:menu_status)";
  ...
  public function updateMenuDetails($data){ 
    try{ 
      ...
      $sql = "UPDATE ".DB_PREFIX."header_menus set  menu_name = :menu_name,menu_type=:menu_type,menu_module=:menu_module,menu_status = :menu_status WHERE menu_id = :menu_id";
  ...
  public function getMenuDetails($data){ 
    try{ 
      extract($data);
      $sql = "SELECT * FROM ".DB_PREFIX."header_menus WHERE `menu_id` = :menu_id";      
      $stmt = $this->connection->prepare($sql);
      $stmt->bindParam(":menu_id",$menu_id);
      $stmt->execute();
  ...
  public function deleteMenuItems($data){ 
    try{ 
      extract($data);
      $sql = "UPDATE ".DB_PREFIX."header_menus SET menu_status = '9' WHERE `menu_id` = :menu_id";      
      $stmt = $this->connection->prepare($sql);
      $stmt->bindParam(":menu_id",$menu_id);
      $res =  $stmt->execute();
      if($res) {
