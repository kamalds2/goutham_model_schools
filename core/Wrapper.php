    ...
    */
    public function __construct() {        
        ...
        if(isset($_GET['controller']) && $_GET['controller'] != '') { 
            ...
            $methodName = isset($_GET['method']) && $_GET['method'] != '' ? $_GET['method'] : 'index';
            $data = isset($_GET['data']) && $_GET['data'] != '' ? $_GET['data'] : array();
            // var_dump($data);die();
        } 
        else { 
            // Use URL parsing logic
            $link = str_replace(SITEURL, '', URL);
            $link = rtrim($link, "/");
            $link = filter_var($link, FILTER_SANITIZE_URL);
            ...
            if($link != 'index.php' && $link != '') {            
                $link = explode('/', $link);
                $controllerName = $link[0];
                $methodName = isset($link[1]) ? $link[1] : 'index';
            } else {
                $controllerName = DEFAULTCONTROLLER;
                $methodName = 'index';
            }
        ...
        if(isset($controllerName) && $controllerName !='') {
            ...
            $init = new $controllerName(); //controller initialisation  
                             
            if(isset($moduleName)) { // model initialisation
